...
#define PUBLISH_SECURITY_MESSAGE_TOPIC "devices/%s/messages/events/%%24.ct=application%%2Fjson&%%24.ce=utf-8&%%24.ifid=urn%%3Aazureiot%%3ASecurity%%3ASecurityAgent%%3A1"
...
{
    CHAR mqtt_publish_topic[MQTT_TOPIC_NAME_LENGTH] = { 0 };
...
static UINT mqtt_init(CHAR * iot_device_id)
{
    ...
    if (status != NX_SUCCESS)
    {
        printf("Failed to create Cloud Helper\r\n");
        return status;
    }
...
static UINT mqtt_open(CHAR *iot_hub_hostname, CHAR *iot_device_id, CHAR *iot_sas_key)
{
    ...
    CHAR mqtt_subscribe_topic[MQTT_TOPIC_NAME_LENGTH];
